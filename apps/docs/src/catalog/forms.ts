...
export const formPages = [
  ...
  page({
    ...
    categoryLabel: "Forms",
    title: "Combobox",
    description: "Let people enter a custom value or quickly reuse a known option from a filterable, keyboard-accessible suggestion list.",
    searchTerms: "combobox autocomplete listbox filter search option active descendant form",
    plugins: ["combobox"],
    body: `<section class="docs-prose-section"><h2>Use it when suggestions help but do not limit input</h2><p>The visible text field remains the user-owned value. Selecting a suggestion can also synchronize a stable identifier to a hidden field, while an unmatched entry remains valid free text. Nyx manages filtering, keyboard movement, popup positioning, selection, and dismissal; your application owns the option source and validation. When a value must come from the list, use Searchable Select instead.</p></section>${card("Deployment location", selectMarkup(comboboxMarkup, ["[data-nyx-example='deployment-location']"]), "Registry source")}`,
  }),
  page({
    ...
    description: "Filter a long directory of known values while ensuring the submitted value comes from an available option.",
    searchTerms: "searchable select combobox choice native select long options",
    plugins: ["combobox"],
    body: `<section class="docs-prose-section"><h2>Use it when search helps people choose one known value</h2><p>This select-only Combobox configuration restores the last committed option when unmatched text is dismissed, so the visible label and hidden submitted ID cannot drift apart. Prefer a native <code>select</code> for a short list. Use the free-text Combobox when custom values are valid.</p></section>${card("Assign an owning team", selectMarkup(comboboxMarkup, ["[data-nyx-example='owning-team']"]), "Combobox configuration")}`,
  }),
  page({
