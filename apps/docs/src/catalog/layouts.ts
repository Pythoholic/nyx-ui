...
export const layoutPages = [
  ...
  page({
    ...
    body: `<section class="docs-prose-section"><h2>Validation and ownership</h2><p>The browser validates required text, email syntax, minimum password length, and consent before submission. Inline guidance appears after a field is edited or the form is submitted. Untouched fields remain neutral; native reset returns the form to its pristine state. Password strength is advisory; it does not replace a server-side password policy.</p><p>The application owns duplicate-account checks, email verification, terms versioning, secure submission, and mapping server errors back to the appropriate field. Add fields only when they are required to create the account.</p></section>${card("Create account", registrationMarkup, "Registry source")}`,
  }),
  page({
