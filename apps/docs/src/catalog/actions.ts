...
export const actionPages = [
  page({
    ...
    body: `<div class="docs-grid">
      ...
      ${card("Icon controls", `<div class="docs-row"><button class="nyx-button nyx-icon-button" aria-label="Add item">${icon("plus")}</button><button class="nyx-button nyx-icon-button" aria-label="Settings">${icon("settings")}</button><button class="nyx-button nyx-icon-button" aria-label="Account">${icon("user")}</button></div>`, "Named")}
    </div>`,
  }),
  page({
