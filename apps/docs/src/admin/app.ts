...
function avatar(name: string): string {
  ...
  return slot?.src ? `<img class="admin-avatar" src="${esc(slot.src)}" alt="${esc(name)}" />` : `<span class="admin-avatar" aria-hidden="true">${esc(initials(name))}</span>`;
}
...
const badge = (status: string) => `<span class="admin-badge" data-tone="${['Paid', 'Active', 'Done', 'Owner'].includes(status) ? 'success' : ['Pending', 'Review', 'Low stock', 'Admin'].includes(status) ? 'warning' : ['Refunded', 'Out of stock'].includes(status) ? 'danger' : 'neutral'}">${esc(status)}</span>`;
...
const sectionHead = (title: string, sub: string, action = '') => `<div class="admin-section-head"><div><h2>${title}</h2><p>${sub}</p></div>${action}</div>`;
const empty = (title: string, sub: string) => `<div class="admin-empty">${icon('search')}<h3>${title}</h3><p>${sub}</p></div>`;

root.innerHTML = `<a class="admin-skip" href="#main">Skip to content</a>
  <aside class="admin-sidebar" id="admin-sidebar" aria-label="Workspace sidebar">
    ...
    <div class="admin-workspace"><span class="admin-avatar">S</span><div><strong data-workspace-name>${esc(state.settings.workspace)}</strong><small>Commerce & operations</small></div></div>
    <nav aria-label="Admin navigation">${[['Workspace', routes.slice(0, 5)], ['Collaborate', routes.slice(5, 8)], ['Manage', routes.slice(8)]].map(([label, items]) => `<p class="admin-nav-label">${label}</p>${(items as string[]).map(id => `<a href="#${id}" data-route="${id}">${icon(id)}<span>${labels[id]}</span>${id === 'inbox' ? '<span class="admin-count" data-unread></span>' : ''}</a>`).join('')}`).join('')}</nav>
    <div class="admin-sidebar-foot"><span class="admin-online">All systems operational</span><a href="${base}">Explore Nyx components ${icon('arrow')}</a><button class="admin-profile" type="button" data-action="profile">${avatar('Alex Morgan')}<span><strong>Alex Morgan</strong><small>Workspace owner</small></span>${icon('settings')}</button></div>
  </aside>
  <div class="admin-shell"><header class="admin-topbar"><button class="admin-icon-button" data-action="menu" aria-label="Toggle navigation" aria-controls="admin-sidebar" aria-expanded="false">${icon('menu')}</button><span class="admin-breadcrumb">Workspace <span>/</span> <strong id="crumb">Overview</strong></span><div class="admin-top-actions"><button class="admin-search-button" data-action="search">${icon('search')}<span>Search workspace</span><kbd>Ctrl K</kbd></button><label class="admin-theme-label"><span class="nyx-visually-hidden">Accent theme</span><select aria-label="Accent theme" id="admin-theme">${['solar', 'signal', 'flux', 'plasma'].map(t => `<option ${state.theme === t ? 'selected' : ''}>${t}</option>`).join('')}</select></label><a href="#inbox" class="admin-icon-button" aria-label="Open inbox">${icon('bell')}<i data-notification-dot></i></a></div></header>
  ...
  <div class="nyx-toast-region" id="admin-toasts"></div><dialog class="admin-dialog" id="admin-dialog" data-nyx-dialog aria-labelledby="dialog-title"></dialog>`;
...
function chart(): string {
  ...
  return `<div class="admin-chart"><svg viewBox="0 0 670 258" role="img" aria-labelledby="revenue-title"><title id="revenue-title">Monthly revenue, ${months.map((m, i) => `${m} ${money(values[i]!)}`).join(', ')}</title><defs><linearGradient id="area" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="var(--nyx-accent)" stop-opacity=".2"/><stop offset="100%" stop-color="var(--nyx-accent)" stop-opacity="0"/></linearGradient></defs>${[0, 1, 2, 3].map(i => `<line x1="50" y1="${40 + i * 170 / 3}" x2="635" y2="${40 + i * 170 / 3}" stroke="var(--nyx-line)" stroke-dasharray="3 5"/><text x="0" y="${44 + i * 170 / 3}">${money(max * (1 - i / 3))}</text>`).join('')}<path d="M50 210 L${points.join(' L')} L630 210 Z" fill="url(#area)"/><polyline points="${points.join(' ')}" fill="none" stroke="var(--nyx-accent)" stroke-width="3" stroke-linejoin="round"/>${points.map((p, i) => `<circle cx="${p.split(',')[0]}" cy="${p.split(',')[1]}" r="4" fill="var(--nyx-panel)" stroke="var(--nyx-accent)" stroke-width="2"><title>${months[i]}: ${money(values[i]!)}</title></circle><text x="${p.split(',')[0]}" y="245" text-anchor="middle">${months[i]}</text>`).join('')}</svg></div>`;
}
function kpis(): string {
  ...
  ].map(([label, value, note, glyph], i) => `<article class="admin-kpi"><div><span>${label}</span>${icon(glyph!)}</div><strong>${value}</strong><footer><span>${note}</span><svg aria-hidden="true" viewBox="0 0 90 26"><polyline points="0,${22-i*2} 12,16 23,20 37,8 48,14 60,7 73,12 90,3" fill="none" stroke="var(--nyx-accent)" stroke-width="2"/></svg></footer></article>`).join('')}</div>`;
}
function revenuePanel(): string { return `<section class="admin-panel">${sectionHead('Revenue overview', 'Paid orders · USD', `<label class="admin-inline-label">Period <select id="period"><option value="6" ${period === '6' ? 'selected' : ''}>Last 6 months</option><option value="3" ${period === '3' ? 'selected' : ''}>Last 3 months</option></select></label>`)}${chart()}<div class="admin-chart-note"><span><i></i> Revenue</span><span>September follows your demo orders</span></div></section>`; }
function paymentRing(): string {
  const percent = Math.round(state.orders.filter(o => o.status === 'Paid').length / (state.orders.length || 1) * 100);
  return `<div class="admin-ring"><svg viewBox="0 0 160 160" aria-hidden="true"><circle cx="80" cy="80" r="70" fill="none" stroke="var(--nyx-line)" stroke-width="10"/><circle cx="80" cy="80" r="70" fill="none" stroke="var(--nyx-accent)" stroke-width="10" pathLength="100" stroke-dasharray="${percent} 100" stroke-linecap="round" transform="rotate(-90 80 80)"/></svg><div><strong>${percent}%</strong><span>Paid orders</span></div></div>`;
}
function overview(): string {
  return `${kpis()}<div class="admin-columns">${revenuePanel()}<section class="admin-panel">${sectionHead('Order health', 'Payment status at a glance')}${paymentRing()}<div class="admin-legend">${['Paid', 'Pending', 'Refunded'].map(s => `<div>${badge(s)}<strong>${state.orders.filter(o => o.status === s).length}</strong></div>`).join('')}</div></section></div>
    <div class="admin-columns"><section class="admin-panel">${sectionHead('Recent orders', 'Your latest customer transactions', '<a class="admin-text-link" href="#orders">View all orders →</a>')}${orderTable(state.orders.slice(0, 5), false)}</section><section class="admin-panel admin-campaign">${imageSlot('campaign')}<div><span class="admin-eyebrow">UP NEXT / SEPTEMBER 28</span><h2>Make room for what’s next.</h2><p>Your autumn collection is taking shape. Bring the team, products, and launch plan together.</p><a href="#projects" class="admin-text-link">Open launch projects ${icon('arrow')}</a></div></section></div>
    <div class="admin-columns equal"><section class="admin-panel">${sectionHead('Team activity', 'The latest across your workspace')}<div class="admin-activity">${[['Sofia Chen', 'Updated the autumn collection', '12 minutes ago'], ['James Wilson', 'Completed checkout refresh', '48 minutes ago'], ['Maya Patel', 'Added a photography brief', '2 hours ago']].map(([name, action, time]) => `<div>${avatar(name!)}<div><strong>${name}</strong><p>${action}</p><small>${time}</small></div></div>`).join('')}</div></section><section class="admin-panel">${sectionHead('Upcoming schedule', 'Keep your next milestones in sight', '<a class="admin-text-link" href="#calendar">Calendar →</a>')}<div class="admin-agenda">${[...state.events].filter(e => e.date >= '2026-09-21').sort((a, b) => `${a.date} ${a.time}`.localeCompare(`${b.date} ${b.time}`)).slice(0, 3).map(e => `<div><span class="admin-date-tile"><small>${new Date(`${e.date}T12:00:00`).toLocaleDateString('en-US', { month: 'short' }).toUpperCase()}</small>${esc(e.date.slice(-2))}</span><div><strong>${esc(e.name)}</strong><p>${esc(e.time)} · ${esc(state.settings.timezone)}</p></div></div>`).join('')}</div></section></div>`;
}
function analytics(): string { return `${kpis()}${revenuePanel()}<div class="admin-columns equal"><section class="admin-panel">${sectionHead('Acquisition channels', 'Illustrative session distribution')}<div class="admin-bars">${[['Organic search', 46], ['Direct', 28], ['Social', 17], ['Referral', 9]].map(([name, v]) => `<div><div><span>${name}</span><strong>${v}%</strong></div><div class="nyx-progress" role="progressbar" aria-label="${name}" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${v}"><span class="nyx-progress-bar" style="--nyx-progress:${v}%"></span></div></div>`).join('')}</div></section><section class="admin-panel">${sectionHead('Top markets', 'Illustrative share of sessions')}<div class="admin-market-list">${[['United States', 'US', '38%'], ['Japan', 'JP', '26%'], ['Germany', 'DE', '19%'], ['United Kingdom', 'GB', '17%']].map(([country, code, v]) => `<div><span class="admin-avatar">${code}</span><span>${country}</span><strong>${v}</strong></div>`).join('')}</div></section></div>`; }
function orderTable(items: RecordItem[], selectable: boolean): string {
  ...
  return `<div class="admin-table-scroll"><table class="admin-table"><thead><tr>${selectable ? '<th><input type="checkbox" id="select-all" aria-label="Select visible orders" /></th>' : ''}<th>Order</th><th>Customer</th><th>Status</th><th class="numeric">Amount</th><th>Date</th><th><span class="nyx-visually-hidden">Actions</span></th></tr></thead><tbody>${items.map(o => `<tr>${selectable ? `<td><input type="checkbox" data-select="${o.id}" ${selected.has(o.id) ? 'checked' : ''} aria-label="Select ${o.id}" /></td>` : ''}<td><button class="admin-text-link" data-edit-order="${o.id}">${o.id}</button></td><td><div class="admin-person">${avatar(o.name)}<span><strong>${esc(o.name)}</strong><small>${esc(o.email)}</small></span></div></td><td>${badge(o.status)}</td><td class="numeric">${money(o.total)}</td><td class="admin-muted">${o.date.slice(5)}</td><td><button class="admin-icon-button" data-edit-order="${o.id}" aria-label="Open ${o.id}">${icon('arrow')}</button></td></tr>`).join('')}</tbody></table></div>`;
}
...
function customers(): string {
  ...
  return `<section class="admin-panel"><div class="admin-toolbar"><label class="admin-filter-search">${icon('search')}<input type="search" id="table-search" aria-label="Search customers" placeholder="Search customers" value="${esc(query)}" /></label><span>${rows.length} customers</span></div>${rows.length ? `<div class="admin-table-scroll"><table class="admin-table"><thead><tr><th>Customer</th><th>Relationship</th><th>Orders</th><th class="numeric">Lifetime spend</th><th>Details</th></tr></thead><tbody>${rows.map(o => `<tr><td><div class="admin-person">${avatar(o.name)}<span><strong>${esc(o.name)}</strong><small>${esc(o.email)}</small></span></div></td><td>${badge('Active')}</td><td>${state.orders.filter(x => x.email === o.email).length}</td><td class="numeric">${money(state.orders.filter(x => x.email === o.email && x.status === 'Paid').reduce((s, x) => s + x.total, 0))}</td><td><button class="admin-text-link" data-customer="${esc(o.email)}">View profile →</button></td></tr>`).join('')}</tbody></table></div>` : empty('No customers found', 'Try another name or email address.')}</section>`;
}
function products(): string { const products = state.products.filter(p => `${p.name} ${p.category}`.toLowerCase().includes(query.toLowerCase())); return `<div class="admin-toolbar standalone"><label class="admin-filter-search">${icon('search')}<input id="table-search" type="search" aria-label="Search products" placeholder="Search products" value="${esc(query)}" /></label><span>${products.length} products</span></div><div class="admin-product-grid">${products.map(p => `<article class="admin-panel admin-product">${imageSlot(p.id)}<div><div class="admin-product-meta"><small>${esc(p.category)}</small>${badge(p.stock === 0 ? 'Out of stock' : p.stock < 10 ? 'Low stock' : 'Active')}</div><h2>${esc(p.name)}</h2><p>${p.stock} available</p><footer><strong>${money(p.price)}</strong><button class="nyx-button" data-edit-product="${p.id}">Edit product</button></footer></div></article>`).join('')}</div>${!products.length ? empty('No products found', 'Try a different product or category.') : ''}`; }
function projects(): string { return `<div class="admin-board">${['Planned', 'In progress', 'Review', 'Done'].map(status => `<section class="admin-board-column"><h2>${status}<span>${state.projects.filter(p => p.status === status).length}</span></h2>${state.projects.filter(p => p.status === status).map(p => `<article class="admin-project"><span class="admin-eyebrow">PROJECT / ${p.id.slice(0, 4)}</span><h3>${esc(p.name)}</h3><div class="admin-progress-label"><span>Completion</span><strong>${p.progress}%</strong></div><div class="nyx-progress" role="progressbar" aria-label="${esc(p.name)} completion" aria-valuenow="${p.progress}" aria-valuemin="0" aria-valuemax="100"><span class="nyx-progress-bar" style="--nyx-progress:${p.progress}%"></span></div><div class="admin-project-owner">${avatar(p.owner)}<span>${esc(p.owner)}<small>Due ${esc(p.due)}</small></span></div><label class="admin-field">Move to<select data-project-status="${p.id}" aria-label="${esc(p.name)} status">${['Planned', 'In progress', 'Review', 'Done'].map(s => `<option ${s === status ? 'selected' : ''}>${s}</option>`).join('')}</select></label><button class="admin-text-link" data-edit-project="${p.id}">Edit project →</button></article>`).join('')}${!state.projects.some(p => p.status === status) ? '<p class="admin-muted">No projects in this stage.</p>' : ''}</section>`).join('')}</div>`; }
function inbox(): string { const message = state.messages.find(m => m.id === activeMessage); return `<section class="admin-panel admin-inbox"><div class="admin-message-list"><h2>Messages <span>${state.messages.length}</span></h2>${state.messages.map(m => `<button data-message="${m.id}" aria-pressed="${m.id === activeMessage}"><div>${avatar(m.name)}<strong>${esc(m.name)}</strong>${m.unread ? '<i aria-label="Unread"></i>' : ''}</div><span>${esc(m.subject)}</span></button>`).join('')}</div><div class="admin-message-body">${message ? `<span class="admin-eyebrow">CONVERSATION</span><h2>${esc(message.subject)}</h2><div class="admin-person">${avatar(message.name)}<span><strong>${esc(message.name)}</strong><small>September 21, 2026</small></span></div><p>${esc(message.body)}</p>${message.replies.map(r => `<div class="admin-reply"><strong>You</strong><p>${esc(r)}</p><small>Saved in this demo</small></div>`).join('')}<form id="reply-form"><label class="admin-field">Reply<textarea name="reply" required rows="4" placeholder="Write a reply…"></textarea></label><button class="nyx-button" data-variant="primary">Save demo reply ${icon('arrow')}</button><small>Demo only · no message is sent</small></form>` : empty('No conversation selected', 'Select a message to read and reply.')}</div></section>`; }
function calendar(): string {
  ...
  return `<section class="admin-panel">${sectionHead(title, `Events shown in ${esc(state.settings.timezone)}`, `<div class="admin-row">${button('←', 'month-prev')}${button('September 2026', 'month-reset')}${button('→', 'month-next')}</div>`)}<div class="admin-calendar-scroll"><div class="admin-calendar">${['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map(d => `<div class="admin-weekday">${d}</div>`).join('')}${Array.from({ length: Math.ceil((start + count) / 7) * 7 }, (_, i) => {
    const day = i - start + 1;
    const date = `${calendarYear}-${String(calendarMonth + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
    return day < 1 || day > count ? '<div class="admin-day outside"></div>' : `<div class="admin-day" ${date === '2026-09-21' ? 'data-today' : ''}><button data-date="${date}" aria-label="Add event on ${date}">${day}</button>${state.events.filter(e => e.date === date).map(e => `<button class="admin-event" data-event="${e.id}"><small>${e.time}</small>${esc(e.name)}</button>`).join('')}</div>`;
  }).join('')}</div></div></section>`;
}
function team(): string { return `<section class="admin-panel">${sectionHead('People & permissions', 'Roles in this demo do not enforce server-side access.')}<div class="admin-table-scroll"><table class="admin-table"><thead><tr><th>Member</th><th>Role</th><th>Status</th><th>Manage access</th></tr></thead><tbody>${state.members.map(m => `<tr><td><div class="admin-person">${avatar(m.name)}<span><strong>${esc(m.name)}</strong><small>${esc(m.email)}</small></span></div></td><td>${badge(m.role)}</td><td>${badge('Active')}</td><td>${m.role === 'Owner' ? '<span class="admin-muted">Workspace owner</span>' : `<select data-role="${m.id}" aria-label="Role for ${esc(m.name)}">${['Admin', 'Member', 'Viewer'].map(r => `<option ${r === m.role ? 'selected' : ''}>${r}</option>`).join('')}</select>`}</td></tr>`).join('')}</tbody></table></div></section>`; }
function billing(): string { return `<div class="admin-columns equal"><section class="admin-panel admin-plan"><span class="admin-eyebrow">CURRENT PLAN</span><h2>Studio <span>Demo subscription</span></h2><p class="admin-price">$49 <small>/ month</small></p><p>Includes 10 team members, unlimited projects, and 100 GB storage.</p><div class="admin-bars"><div><div><span>Team seats</span><strong>${state.members.length} / 10</strong></div><progress max="10" value="${state.members.length}" aria-label="Team seats"></progress></div></div>${button('View plan details', 'plan')}</section><section class="admin-panel">${sectionHead('Billing details', 'Illustrative billing information')}<dl class="admin-definition"><div><dt>Business</dt><dd>${esc(state.settings.workspace)}</dd></div><div><dt>Billing email</dt><dd>${esc(state.settings.email)}</dd></div><div><dt>Payment method</dt><dd>Visa ending 4242 · sample card</dd></div><div><dt>Next invoice</dt><dd>October 1, 2026</dd></div></dl><a href="#settings" class="admin-text-link">Update workspace details →</a></section></div><section class="admin-panel">${sectionHead('Invoices', 'Download sample invoices for your records')}<div class="admin-table-scroll"><table class="admin-table"><thead><tr><th>Invoice</th><th>Date</th><th>Status</th><th>Amount</th><th>Download</th></tr></thead><tbody>${['September', 'August', 'July'].map((m, i) => `<tr><td>INV-2026-0${9-i}</td><td>${m} 1, 2026</td><td>${badge('Paid')}</td><td>$49.00</td><td><button class="admin-text-link" data-invoice="${m}">Download ${icon('download')}</button></td></tr>`).join('')}</tbody></table></div></section>`; }
function settings(): string { return `<div class="admin-columns"><section class="admin-panel"><form id="settings-form">${sectionHead('Workspace settings', 'Personalize your workspace and notification preferences.')}<div class="admin-form-grid"><label class="admin-field">Workspace name<input name="workspace" required maxlength="60" value="${esc(state.settings.workspace)}" /></label><label class="admin-field">Contact email<input name="email" required type="email" value="${esc(state.settings.email)}" /></label><label class="admin-field">Timezone<select name="timezone">${['Asia/Tokyo', 'America/New_York', 'Europe/London', 'UTC'].map(t => `<option ${state.settings.timezone === t ? 'selected' : ''}>${t}</option>`).join('')}</select></label><label class="admin-check"><input type="checkbox" name="digest" ${state.settings.digest ? 'checked' : ''} /> Weekly activity digest <small>Preference saved locally; email delivery is not connected.</small></label></div><div class="admin-form-footer"><button class="nyx-button" data-variant="primary">Save changes</button></div></form></section><section class="admin-panel">${sectionHead('Demo controls', 'Explore states safely with sample data.')}<p class="admin-muted">Changes persist in this browser. Reset restores the original records, preferences, and theme.</p>${button('Reset demo data', 'reset')}<hr/><h3>Account screens</h3><p class="admin-muted">Preview sign-in and recovery layouts.</p>${button('Preview sign in', 'signin')}</section></div>`; }
...
function render(): void {
  ...
  main.innerHTML = `<div class="admin-page-heading"><div><p class="admin-eyebrow">${esc(state.settings.workspace)} <span>/</span> ${route === 'overview' ? 'MONDAY, SEPTEMBER 21, 2026' : 'WORKSPACE'}</p><h1>${route === 'overview' ? 'Workspace overview' : labels[route]}</h1><p>${descriptions[route]}</p></div><div class="admin-heading-actions">${['overview', 'analytics', 'orders', 'customers'].includes(route) ? button('Export orders', 'export', false, 'download') : ''}${create[route] ? button(create[route]!, 'create', true, 'plus') : route === 'overview' ? button('Create order', 'new-order', true, 'plus') : ''}</div></div>${({ overview, analytics, orders, customers, products, projects, inbox, calendar, team, billing, settings } as Record<string, () => string>)[route]!()}`;
...
function openDialog(title: string, body: string, submit?: (form: HTMLFormElement) => void): void {
  ...
  dialog?.destroy();
  el.innerHTML = `<header><div><span class="admin-eyebrow">NYX WORKSPACE</span><h2 id="dialog-title">${esc(title)}</h2></div><button class="admin-icon-button" type="button" data-nyx-dialog-close aria-label="Close dialog">${icon('close')}</button></header>${body}`;
...
const field = (name: string, label: string, value = '', type = 'text') => `<label class="admin-field">${label}<input name="${name}" type="${type}" required value="${esc(value)}" ${type === 'number' ? 'min="0" step="0.01"' : 'maxlength="120"'} /></label>`;
const selectField = (name: string, label: string, values: string[], current = '') => `<label class="admin-field">${label}<select name="${name}">${values.map(v => `<option ${v === current ? 'selected' : ''}>${v}</option>`).join('')}</select></label>`;
...
function searchDialog(): void {
  openDialog('Search workspace', `<label class="admin-field">Search pages, orders, and products<input id="global-search" type="search" placeholder="Try orders, Sofia, or headphones" autocomplete="off" /></label><div id="search-results" class="admin-search-results"></div>`);
...
root.addEventListener('click', event => {
  ...
  if (target.dataset.customer) { const o = state.orders.find(x => x.email === target.dataset.customer)!; openDialog(o.name, `<div class="admin-person">${avatar(o.name)}<span>${esc(o.email)}</span></div><h3>Order history</h3>${orderTable(state.orders.filter(x => x.email === o.email), false)}`); }
  ...
  switch (target.dataset.action) {
    case 'menu': { const open = root.classList.toggle('nav-open'); target.setAttribute('aria-expanded', String(open)); break; }
    ...
    case 'signin': openDialog('Welcome back', `<form><p>Sign in to your workspace.</p><div class="admin-form-grid">${field('email', 'Email', '', 'email')}${field('password', 'Password', '', 'password')}</div><div class="admin-form-footer"><button class="nyx-button" data-variant="primary">Preview sign in</button></div><p>Authentication preview · no credentials are sent or saved.</p></form><button class="admin-text-link" type="button" data-action="recovery">Forgot password?</button>`, () => { dialog?.close(); notify('Sign-in preview complete', 'Connect your identity provider for real authentication.', 'neutral'); }); break;
...
function navigate(): void { const hash = location.hash.slice(1).split('?')[0]!; route = routes.includes(hash) ? hash : 'overview'; query = ''; filter = 'All'; page = 1; selected.clear(); root.classList.remove('nav-open'); root.querySelector('[data-action=menu]')!.setAttribute('aria-expanded', 'false'); render(); window.scrollTo(0, 0); }
window.addEventListener('hashchange', () => { navigate(); main.focus({ preventScroll: true }); });
document.addEventListener('keydown', event => { if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 'k') { event.preventDefault(); searchDialog(); } if (event.key === 'Escape') { root.classList.remove('nav-open'); root.querySelector('[data-action=menu]')!.setAttribute('aria-expanded', 'false'); } });
navigate();
