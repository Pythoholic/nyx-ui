...
root.innerHTML = `<a class="admin-skip" href="#main">Skip to content</a>
  ...
  <div class="nyx-app-content admin-shell"><main id="main" class="nyx-app-main" tabindex="-1"></main><footer class="admin-footer"><span>Nyx Stealth admin workspace</span><span>Sample data · changes stay in this browser</span></footer><div class="nyx-toast-region" id="admin-toasts"></div><dialog class="nyx-dialog admin-dialog" id="admin-dialog" data-nyx-dialog aria-labelledby="dialog-title"></dialog></div>`;
...
function overview(): string {
  return `${kpis()}<div class="admin-columns">${revenuePanel()}<section class="nyx-panel admin-panel admin-health-panel">${sectionHead('Order health', 'Payment status at a glance')}<div class="admin-health-body">${paymentRing()}<div class="admin-legend" aria-label="Order status counts">${['Paid', 'Pending', 'Refunded'].map(s => `<div>${badge(s)}<strong>${state.orders.filter(o => o.status === s).length}</strong></div>`).join('')}</div></div></section></div>
    <div class="admin-columns"><section class="nyx-panel admin-panel">${sectionHead('Recent orders', 'Your latest customer transactions', routeButton('View all orders', 'orders'))}${orderTable(state.orders.slice(0, 5), false)}</section><section class="nyx-panel admin-panel admin-campaign">${imageSlot('campaign')}<div><span class="nyx-eyebrow">UP NEXT / SEPTEMBER 28</span><h2 class="nyx-panel-title">Make room for what’s next.</h2><p>Your autumn collection is taking shape. Bring the team, products, and launch plan together.</p>${routeButton('Open launch projects', 'projects')}</div></section></div>
