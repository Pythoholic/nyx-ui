...
| The catalog is approximately 95 routes | **Does not match the current repository description.** | The root README currently says 86 component routes. That count is not a sound reason by itself to select a version; release maturity and compatibility intent are the relevant factors. |
...
| The source repository is ready for public provenance | **Not established, and currently appears false.** | Both manifests and the Git remote point to `Pythoholic/nyx-stealth`, but an unauthenticated request for that GitHub repository returned 404. That means the source is not publicly resolvable at review time, whether because it is private or because the remote is not a public repository. npm automatically emits provenance through Trusted Publishing only for public packages built from a public repository ([Trusted Publishing](https://docs.npmjs.com/trusted-publishers/)). |
